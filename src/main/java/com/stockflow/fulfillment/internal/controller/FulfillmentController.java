...
class FulfillmentController {
    ...
    public ApiResponse<FulfillmentTask> create(@Valid @RequestBody CreateTaskRequest request,
                                               @AuthenticatedUser CurrentUser actor) {
        return ApiResponse.ok(fulfillment.createTask(request.orderId(), request.assignedUserId(), actor));
    }
    ...
    public ApiResponse<List<FulfillmentTask>> list(@AuthenticatedUser CurrentUser actor) {
        return ApiResponse.ok(fulfillment.listTasks(actor));
    }

    @PutMapping("/{taskId}/assignment")
    @RequiresPermission(resource = FulfillmentResources.PICK_LISTS, action = Action.UPDATE)
    @PreAuthorize("hasAnyAuthority('" + Roles.ORDER_COORDINATOR + "','" + Roles.WAREHOUSE_MANAGER + "')")
    public ApiResponse<FulfillmentTask> assign(@PathVariable UUID taskId,
            @Valid @RequestBody AssignmentRequest request, @AuthenticatedUser CurrentUser actor) {
        return ApiResponse.ok(fulfillment.assign(taskId, request.assignedUserId(), actor));
    }

    @GetMapping("/{taskId}")
    @RequiresPermission(resource = FulfillmentResources.PICK_LISTS, action = Action.READ)
    public ApiResponse<FulfillmentTask> find(@PathVariable UUID taskId,
                                             @AuthenticatedUser CurrentUser actor) {
        return ApiResponse.ok(fulfillment.findTask(taskId, actor));
    }

    @PostMapping("/{taskId}/picking-start")
    @RequiresPermission(resource = FulfillmentResources.PICK_LISTS, action = Action.UPDATE)
    @PreAuthorize("hasAnyAuthority('" + Roles.WAREHOUSE_STAFF + "','" + Roles.WAREHOUSE_MANAGER + "')")
    public ApiResponse<FulfillmentTask> startPicking(@PathVariable UUID taskId,
                                                     @AuthenticatedUser CurrentUser actor) {
        return ApiResponse.ok(fulfillment.startPicking(taskId, actor));
    }

    @PostMapping("/{taskId}/picking-completion")
    @RequiresPermission(resource = FulfillmentResources.PICK_LISTS, action = Action.UPDATE)
    @PreAuthorize("hasAnyAuthority('" + Roles.WAREHOUSE_STAFF + "','" + Roles.WAREHOUSE_MANAGER + "')")
    public ApiResponse<FulfillmentTask> completePicking(@PathVariable UUID taskId,
                                                        @AuthenticatedUser CurrentUser actor) {
        return ApiResponse.ok(fulfillment.completePicking(taskId, actor));
    }
    ...
    @PreAuthorize("hasAnyAuthority('" + Roles.WAREHOUSE_STAFF + "','" + Roles.WAREHOUSE_MANAGER
            + "','" + Roles.QC_STAFF + "')")
    public ResponseEntity<ApiResponse<FulfillmentArtifactAccess>> artifactDownload(
            @PathVariable UUID taskId, @PathVariable UUID lineId, @PathVariable String role,
            @AuthenticatedUser CurrentUser actor) {
        return ResponseEntity.ok().cacheControl(CacheControl.noStore())
                .body(ApiResponse.ok(fulfillment.artifactDownload(taskId, lineId, role, actor)));
    }

    @PostMapping("/{taskId}/packing-completion")
    @RequiresPermission(resource = FulfillmentResources.PACKAGES, action = Action.CREATE)
    @PreAuthorize("hasAnyAuthority('" + Roles.WAREHOUSE_STAFF + "','" + Roles.WAREHOUSE_MANAGER + "')")
    public ApiResponse<FulfillmentTask> completePacking(@PathVariable UUID taskId,
                                                        @AuthenticatedUser CurrentUser actor) {
        return ApiResponse.ok(fulfillment.completePacking(taskId, actor));
    }

    @PostMapping("/{taskId}/design-integrity-reverification")
    @RequiresPermission(resource = FulfillmentResources.PACKAGES, action = Action.CREATE)
    @PreAuthorize("hasAnyAuthority('" + Roles.QC_STAFF + "','" + Roles.WAREHOUSE_MANAGER + "')")
    public ApiResponse<FulfillmentTask> reverify(@PathVariable UUID taskId,
            @Valid @RequestBody ReverificationRequest request, @AuthenticatedUser CurrentUser actor) {
        return ApiResponse.ok(fulfillment.reverifyDesignIntegrity(taskId, request.resolutionNote(), actor));
    }
